"""Backend-aware paths for the energy cache + outputs.

On the fleet (``FW_STORAGE=s3`` / ``FW_DATA_ROOT=s3://afl-cache``) artifacts —
the cached EIA energy series + the rendered dashboard HTML — land in the
...
def cache_root() -> str:
    ov = os.environ.get("FW_ENERGY_CACHE_DIR")
    if ov:
        return ov
    r = _data_root()
    return join(r, "cache", "energy", "cache") if is_remote(r) else join(r, "energy-cache")


def output_root() -> str:
    ov = os.environ.get("FW_ENERGY_OUTPUT_DIR")
    if ov:
        return ov
    r = _data_root()
    return join(r, "cache", "energy", "output") if is_remote(r) else join(r, "energy-output")
